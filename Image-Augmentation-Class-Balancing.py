...
def balance_class(
    source_directory, target_directory, imagegen_obj, target_size=None, cmap="viridis"
):
    ...
    for directory in os.listdir(source_directory):
        ...
        class_examples.append(
            len(os.listdir(os.path.join(source_directory, directory)))
        )
        ...
        if target_size > class_examples[i]:
            ...
            for imgpath in tqdm(os.listdir(os.path.join(source_directory, directory))):
                ...
                for i in range(q):
                    if cmap == "gray":
                        aug_img = np.squeeze(
                            dataaug.random_transform(np.expand_dims(img, axis=2))
                        )
                    else:
                        aug_img = imagegen_obj.random_transform(img)
                    plt.imsave(
                        os.path.join(dest_dir, f"{filename}-{i+1}{file_extension}"),
                        aug_img,
                        cmap=cmap,
                    )
                prob = random.random()
                if prob <= r:
                    if cmap == "gray":
                        ...
                        aug_img = imagegen_obj.random_transform(img)
                    plt.imsave(
                        os.path.join(dest_dir, f"{filename}-{0}{file_extension}"),
                        aug_img,
                        cmap=cmap,
                    )
        else:
    ...
    for directory in os.listdir(target_directory):
        class_examples_updated.append(
            len(os.listdir(os.path.join(target_directory, directory)))
        )
    print("Updated Class Distribution:", class_examples_updated)


balance_class(
    source_directory="dataset/train/",
    target_directory="dataset/train-balanced/",
    imagegen_obj=dataaug,
    target_size=None,
)
