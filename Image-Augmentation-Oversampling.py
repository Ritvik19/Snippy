...
def balance_class(source_directory, target_directory, imagegen_obj, target_size=None):
    ...
    for i, directory in enumerate(os.listdir(source_directory)):
        ...
        for imgpath in tqdm(os.listdir(os.path.join(source_directory, directory))):
            ...
            plt.imsave(os.path.join(dest_dir, imgpath), img)
            for i in range(q):
                aug_img = imagegen_obj.random_transform(img)
                plt.imsave(os.path.join(dest_dir, f"{filename}-{i+1}{file_extension}"), aug_img)
            prob = random.random()
            if prob <= r:
                aug_img = imagegen_obj.random_transform(img)
                plt.imsave(os.path.join(dest_dir, f"{filename}-{0}{file_extension}"), aug_img)
